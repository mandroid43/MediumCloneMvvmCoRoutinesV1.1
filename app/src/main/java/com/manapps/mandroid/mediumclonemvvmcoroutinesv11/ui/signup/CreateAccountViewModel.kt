package com.manapps.mandroid.mediumclonemvvmcoroutinesv11.ui.signup
...
import com.manapps.mandroid.mediumclonemvvmcoroutinesv11.R
import com.manapps.mandroid.mediumclonemvvmcoroutinesv11.data.models.entities.SignupData
import com.manapps.mandroid.mediumclonemvvmcoroutinesv11.data.models.request.SignupRequest
import com.manapps.mandroid.mediumclonemvvmcoroutinesv11.data.models.response.UserResponse
import com.manapps.mandroid.mediumclonemvvmcoroutinesv11.data.repository.CreateAccountRepository
import com.manapps.mandroid.mediumclonemvvmcoroutinesv11.utils.*
...
class CreateAccountViewModel(
    ...
    private fun isEmailValid(email: String): Boolean {
        ...
        if (email.isEmpty()) {
            _emailFieldValidation.value =
                Resource.error(null, context.resources.getString(R.string.emptyEmail))
        } else {
            isValidEmail =
                (!TextUtils.isEmpty(email) && Patterns.EMAIL_ADDRESS.matcher(email).matches())
            if (!isValidEmail) {
                _emailFieldValidation.value =
                    Resource.error(null, context.resources.getString(R.string.invalidEmail))
            } else {
    ...
    private fun isPasswordValid(password: String): Boolean {
        ...
        if (password.isEmpty()) {
            _passwordFieldValidation.value =
                Resource.error(null, context.resources.getString(R.string.emptyPassword))
        } else {
            ...
            if (isValidPassword) {
                ...
                _passwordFieldValidation.value =
                    Resource.error(null, context.resources.getString(R.string.invalidPassword))
            }
        }
    ...
    private fun isUserNameValid(userName: String): Boolean {
        ...
        if (userName.isEmpty()) {
            _userNamFieldValidation.value =
                Resource.error(null, context.resources.getString(R.string.emptyUserName))
        } else {
    ...
    fun checkValidationsAndSendCreateAccountRequest(
        userName: String,
        email: String,
        password: String
    ) {
        if (isUserNameValid(userName) && isEmailValid(email) && isPasswordValid(password) && NetworkHelper.isNetworkConnected(
                context
            )
        ) {
    ...
    private fun setSignupError(message: String) {
        ...
        _signupResultLiveData.postValue(
            Resource.error(
                null,
                context.resources.getString(R.string.invalidCreateAccountCredentials)
            )
        )
    }
